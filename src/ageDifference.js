...
function getDifferenceAge() {
    let byName = getObjectByName(ancestry);
    let childrenHavingAMother = getChildrenHavingAMother(ancestry);
    let mothersInList = getMothersInList(childrenHavingAMother, byName);
    let childrenHavingMotherInList = getChildrenHavingMotherInList(mothersInList, byName);
    let ageDifferenceArray = getAgeDifference(childrenHavingMotherInList, mothersInList)

    function getObjectByName(object) {
        let objectByname = {};

        object.forEach(function (person) {
            objectByname[person.name] = person;
        });

        return objectByname;
    };

    function getChildrenHavingAMother(object) {
        let childrenHvingAMother = {};

        object.forEach(function (person) {
            if (person.mother !== null)
                childrenHvingAMother[person.name] = person;
        });

        return childrenHvingAMother;
    };

    function getMothersInList(children, object) {
        let childrenNameHavingAMother = Object.keys(children);
        let mothers = [];
        let mothersInList = {};
        childrenNameHavingAMother.forEach(function (person) {
            mothers.push(object[person].mother);
        });

        mothers.forEach(function (person) {
            if (object[person])
                mothersInList[person] = object[person];
        });

        return mothersInList;
    };

    function getChildrenHavingMotherInList(mothers, object) {
        let mothersNameInList = Object.keys(mothers);
        let ChildrenHavingMotherInList = {}
        for (key in object) {
            if (mothersNameInList.includes(object[key].mother))
                ChildrenHavingMotherInList[key] = object[key];
        }

        return ChildrenHavingMotherInList;
    };

    function getAgeDifference(child, mothers) {
        let difference = [];

        for (key in child) {
            difference.push(child[key].born - mothers[child[key].mother].born);
        }
...
console.log(average(getDifferenceAge()));